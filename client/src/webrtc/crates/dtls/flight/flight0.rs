...
impl Flight for Flight0 {
    async fn parse(
        ...
        cfg: &HandshakeConfig,
    ) -> std::result::Result<Box<dyn Flight + Send + Sync>, (Option<Alert>, Option<Error>)> {
    ...
    ) -> std::result::Result<Vec<Packet>, (Option<Alert>, Option<Error>)> {
        // Initialize
